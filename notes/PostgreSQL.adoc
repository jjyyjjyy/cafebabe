...
文档: https://www.postgresql.org/docs/current/datatype.html[, role="external", window="_blank"]
...
plantuml::charts/postgresql/LogicalStructure.puml[format=svg]

=== 物理结构

plantuml::charts/postgresql/PhysicalStructure.puml[format=svg]
...
* 数据库对象OID保存在 `pg_database` 系统表里.
* 表/索引/序列等对象OID保存在 `pg_class` 系统表里.

==== 表数据文件结构

[rackdiag, pg_table_file, svg, width=400, height=200]
.表数据文件结构
----
rackdiag {
  ascending;
  ...
  description = "8KB per page by default";
...
[bytefield, pg_table_tuple, svg, width=800, height=400]
.page结构
----
(def boxes-per-row 8)
(def box-width 70)
(defattrs :plain {:font-size 12 })
(draw-column-headers)
(draw-box "pd_lsn" {:span 8})
(draw-box "pd_checksum" {:span 2})
(draw-box "pd_flags" {:span 2})
(draw-box "pd_lower" {:span 2})
(draw-box "pd_upper" {:span 2})
(draw-box "pd_special" {:span 2})
(draw-box "pd_pagesize_version" {:span 2})
(draw-box "pd_prune_xid" {:span 4})
(draw-box (text "pd_linp" [:math] [:sub 1]) {:span 4})
(draw-box (text "pd_linp" [:math] [:sub 2]) {:span 4})
(draw-gap "Free space")
(draw-box (text "Tuple" [:math] [:sub 2]) {:span 8})
(draw-box (text "Tuple" [:math] [:sub 1]) {:span 8})
(draw-bottom)
----

. `pd_lower`: 指向free space开始位置
. `pd_upper`: 指向free space结束位置
. 如果Tuple超过page 1/4, 则会放到 `TOAST` page中.
. Tuple可以使用 `TID<pageNumber, offset>` 二元组定位
** pageNumber: page序号.
** offset: 指向第几个line pointer.

=== 进程结构

plantuml::charts/postgresql/ProcessStructure.puml[format=svg]
...
plantuml::charts/postgresql/MemoryStructure.puml[format=svg, width=600, height=400]
